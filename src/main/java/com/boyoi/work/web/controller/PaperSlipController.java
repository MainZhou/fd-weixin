 ...
 */
@Controller
@RequestMapping(value = "paperSlip")
public class PaperSlipController {
    ...
    public String list(HttpServletRequest request, Model model) {
        List<Map<String, String>> list = (List<Map<String, String>>) request.getSession().getAttribute("imgList");
        model.addAttribute("lists", list);
        return "paperSlip/paperSlip";
    }
    ...
    public ResponseResult saveImg(String img, int index,String salesTime ,HttpSession session) {
        ...
        List<Map<String, String>> list = (List<Map<String, String>>) session.getAttribute("imgList");
        if (list != null) {
            if (index != -1) {
    ...
    public ResponseResult addImgs(@RequestBody List<PaperPhoto> imgs, HttpSession session, HttpServletRequest request) {
        ...
        sales.setUserType(((JSONObject) userInfo.get("userInfo")).get("userType").toString());
        ...
        sales.setData(imgs);
        String entrRecordUrl = WxUtils.baseUrl + "/saleRecords/fd_ws_dealSaveByPaperNew";
