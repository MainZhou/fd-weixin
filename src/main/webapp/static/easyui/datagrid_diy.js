...
$.ajax({type:opts.method,url:opts.url,data:_248,dataType:"json",success:function(data){
    //add by chenj 处理没有查询到记录时的显示信息
    var activeTab = Addtabs.activeTabContent();
    var href = $(activeTab + " .subTabs-tag > ul li.active a").attr("href");
    if(href){
        activeTab =  activeTab + ' .subTabs-tag .tab-content ' + href;
    }
    if (data && data.rows == null || data.rows.length == 0) {
        _249(data);

        var _no_result_html_ = "<div style='height:45px;width:100%;margin-top:15px;text-align: center;font-size:15px;font-weight:bold;color:#4889D4;'>没有符合条件的记录！&nbsp;&nbsp;&nbsp;&nbsp;</div>";
        jQuery(activeTab + " .datagrid-view2 .datagrid-body").html (_no_result_html_);//表格区域信息
        jQuery(activeTab + " .pagination-info").html("<span style='color:green;'>没有符合条件的记录！&nbsp;&nbsp;&nbsp;&nbsp;</span>");//右下角信息
    }else{
        _249(data);
    }
}
    ,statusCode: {500: function() {
        ...
        //add by chenj 处理查询出错时的显示信息
        var activeTab = Addtabs.activeTabContent();
        var href = $(activeTab + " .subTabs-tag > ul .active a").attr("href");
        if(href != 'undefined'){
            activeTab =  activeTab + ' .subTabs-tag .tab-content ' + href;
            alert(activeTab)
        }
        var _errMsg_ = "查询记录时出现错误！";
        var _query_error_html_ = "<div style='height:45px;width:100%;margin-top:15px;text-align: center;font-size:15px;font-weight:bold;color:#CC0000;'>"+_errMsg_+"&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</div>";
        jQuery(activeTab +" .datagrid-view2 .datagrid-body").html(_query_error_html_);//表格区域信息
        jQuery(activeTab + " .pagination-info").html("<span style='color:red;'>"+_errMsg_+"</span>");//右下角信息
    }}
    ,error:function(){
_24a.apply(this,arguments);
        //add by chenj 处理查询出错时的显示信息
